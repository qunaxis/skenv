...
commit    = "`+f.git(filepath.Join(f.work, "example-vendor__tools"), "rev-parse", "HEAD")+`"
`)
...
[user.dependencies.diagrams]
repo      = "example-vendor/tools"
skill_dir = "tools/diagrams"
commit    = "` + rev + `"
`,
		"skills/code-review/SKILL.md":    exampleSkill("code-review", "Review a diff for bugs before it is merged.", "Read the whole diff first."),
...
// olderRepo is publicRepo as the templates of skenv 0.5.0 left it: that
// template_version, and the headers of the files generated by 0.5.0.
func (f *exampleWorld) olderRepo() string {
	repo := f.publicRepo()
