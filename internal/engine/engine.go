...
// then `manifest` in ~/.config/skenv/config.{toml,yaml,yml,json}. Without
// any of them it returns ErrNoManifest.
func ResolveManifest(env Env, flag string) (string, error) {
	cfg, err := config.Load(env.Home, env.Getenv, map[string]string{config.Manifest: flag})
	if err != nil {
		return "", err
	}
	if m := cfg.Get(config.Manifest); m != "" {
