...
The `repo` of a checkout or a dependency names a git repository. skenv
reads GitHub, GitLab and Codeberg short forms, aliases for self-hosted servers
...
| `github:owner/repo`          | `github:example-org/skills`                 | `https://github.com/example-org/skills.git`           | github.com, named explicitly                                               |
...
| `<alias>:path`               | `work:platform/skills`                      | `https://git.example.com/platform/skills.git`         | a host declared under [`[user.git_hosts.<alias>]`](#declaring-a-self-hosted-host) |
...
- **Trailing `.git`** is optional in the short forms: `example-org/skills`
  and `example-org/skills.git` are the same repository. For a host with
  provider `generic` the path is used as written, so write `.git` when the server
  needs it.
- **Paths** in the short forms are segments of letters, digits, `.`, `_`
  and `-`. GitHub, Codeberg and hosts with provider `github` or `gitea` take
...
`skenv init` and `import` write the shorter `owner/repo` for github.com.

## Declaring a self-hosted host

Declare the server once, under an alias of your choice, in the `[user]`
section of the skenv file. After that, `work:group/repo` works in
`[user.checkouts.<id>]`, `[user.dependencies.<name>]` and
`skenv vendor add`.
...
```json
{
  "user": {
    ...
    "checkouts": {
      "skills": { "repo": "work:platform/skills", "checkout_dir": "~/src/skills" }
    },
    "dependencies": {
      "deploy": {
        "repo": "work:platform/team/tools",
...
| ---------- | ------ | ----------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `base_url` | string | required    | The base of the repositories on the server. Its scheme is the transport: `https://git.example.com` clones `<base_url>/group/repo.git` over https, `ssh://git@git.example.com` over ssh. It may include a path (`https://git.example.com/scm`) and, for ssh, a port. It must not carry credentials or a password. |
| `provider` | string | `"generic"` | The software of the server; see the table below.                                                                                                                                                                                         |
...
`git_hosts` (git servers) is unrelated to `[user.machines."<name>"]`
([machine rules](manifest.md#machine-rules)).

| `provider` | Servers                  | Accepted paths                        | Clone URL                       |
| ---------- | ------------------------ | ------------------------------------- | ------------------------------- |
| `github`   | GitHub Enterprise Server | `owner/repo`                          | `<base_url>/owner/repo.git`     |
| `gitlab`   | GitLab                   | `group/repo`, subgroups of any depth  | `<base_url>/group/.../repo.git` |
| `gitea`    | Gitea, Forgejo           | `owner/repo`                          | `<base_url>/owner/repo.git`     |
| `generic`  | anything else            | any path                              | `<base_url>/<path>` as written  |
...
The scheme of `base_url` decides how skenv clones:

```toml
[user.git_hosts.work]
base_url = "ssh://git@git.example.com:2222/scm"   # user, optional port and path prefix
provider = "gitlab"
```

Here `work:platform/skills` clones
`ssh://git@git.example.com:2222/scm/platform/skills.git`. To recognise an
existing clone (in `init`, `clone` and `import`), skenv also derives the
counterpart of the base: `git@<host>:` for an https base,
`https://<host>/<prefix>` for an ssh one. A remote in either form maps to
the alias. An https base is the default choice for a manifest shared by
many machines; each machine can still fetch over ssh with `insteadOf`
...
`[user.git_hosts]` and syncs the `work:...` entries exactly as on the
first machine: the same clone URLs, the same `.skenv` markers, the same
links.

A project declares its own hosts under `[project.git_hosts.<alias>]`, with
the same keys, for the `repo` values of `[project]`; it never uses the hosts of
a manifest. See [Project skills](project-skills.md#the-project-section).

## Authentication

skenv clones the built-in short forms (`owner/repo`, `github:`, `gitlab:`,
`codeberg:`) over https, and a declared host over the scheme of its
`base_url`. It never stores or asks for credentials: git does, with the
settings you already use. Never put a token in the manifest; skenv rejects
a `base_url` with credentials in it.

- **https** works with a
  [git credential helper](https://git-scm.com/docs/gitcredentials), for
  example the macOS keychain or your forge's CLI.
- **ssh for a declared host**: set `base_url = "ssh://git@git.example.com"`
  and the key of every machine does the rest.
- **ssh for a built-in host, or on one machine only**: map the https base
  to ssh once per machine with git's
...
- **Dependency cache**: the directory in `~/.cache/skenv/repos` is derived from
  the canonical URL after your `insteadOf` rules (see
  [where the cache lives](manifest.md#where-the-cache-lives)).
- **`.skenv` marker**: the copy of a dependency in the store records the canonical
...
- **`doctor`** shows the canonical URL: `checkout skills
  (work:platform/skills (https://git.example.com/platform/skills.git)) is
  not cloned`, and the `wrong-rev` detail compares the URLs of the store
  and the manifest.

**Moving to a mirror.** When you point an entry at another server, by
changing `repo` or the `base_url` of its host, the canonical URL changes:

- dependencies are copied again from the new server on the next `sync`,
  into a new cache directory. `commit` must exist there too, or `sync`
  stops with `commit ... not found`. The old cache directory can be
  deleted;
- a checkout that already exists keeps its `origin`, and `sync`
...
```text
repo "acme:platform/tools": unknown host prefix "acme:" (known: github:, gitlab:, codeberg:, work:); declare it under [user.git_hosts.acme], or write owner/repo for github.com or a full git URL
```

The prefix is neither built in nor declared. Fix a typo, declare the host
under `[user.git_hosts.acme]`, or write the full URL. An ssh host alias
...
```text
repo "work:platform/skills": unknown host prefix "work:" (known: github:, gitlab:, codeberg:); ...; a host declared in the manifest is not known before it is cloned, so pass the full URL
```
...
Check the path against the `provider` of the host (see the table above).
...
The `insteadOf` hint appears only for a repository cloned over https.
`Could not resolve host` means the `base_url` of the host is wrong or the server
...
repository and adds the repository as its first checkout,
`[user.checkouts.<repository name>]` with `checkout_dir = "."`, its `repo`
written in the short form of its `origin`:
...
| a local path or `file://` URL                         | no checkout                                      |
...
host's `base_url` matches the origin (or its derived counterpart).
